...
\section{trec\_eval}
For assessing the system's performance, we utilized the TREC Eval tool, which was created by the Text REtrieval Conference (TREC) to establish a standardized method for comparing the efficacy of various search engines.
...
The ensuing tables show the outcomes derived from our search engine evaluations, conducted using the queries from the dataset labeled \href{https://msmarco.blob.core.windows.net/msmarcoranking/msmarco-test2020-queries.tsv.gz}{\textit{msmarco-test2020-queries}}.
...
The table presents the query processing times for the provided collection. Additionally, we present the output of the Indexing program, indicating that it takes less than 1 minute and 20 seconds to construct the index previously discussed in the preceding sections.

Furthermore, we have included a histogram illustrating the distribution of execution times for run queries, comparing two methods: BMM and DAAT. This graphical representation allows for a visual comparison of the execution time disparities between the two query processing approaches.
...
\section{Files' size}
The following table shows the file sizes presented in megabytes, segmented into N partitions (in our case, 5 partitions) for each data structures, including Document Index, Local Lexicon, and Posting List. Additionally, the Global Lexicon remains undivided, contributing to a total size of 1034.7 MiB.

To provide a clearer visualization, we have included a tree diagram below where the divisions of megabytes among Document Index, Local Lexicon, Posting List, and Global Lexicon are depicted.
